...
module.exports = (callback = Hoek.ignore) => {
  ...
  async.series(
    ...
    (error, { previousMigrations, allMigrations }) => {
      ...
      const migrationsFilesToRun = allMigrations.filter(
        m => !previousMigrations.includes(m),
      );
...
const getMigrations = client => (callback) => {
  ...
  client.query(query, (error, result) => callback(error, result.rows.map(row => row.migration_file)));
};
...
const addCompletedMigrationsToDb = (client, completedMigrations, callback) => {
  ...
  const querySuffix = completedMigrations.map(m => `('${m}') `).join(', ');
